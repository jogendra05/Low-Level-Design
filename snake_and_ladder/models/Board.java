...
public class Board {
    ...
    private int getCol(int position) {
        ...
        int posInRow = (position - 1) % sideLength;
        boolean leftToRight = ((sideLength - 1 - row) % 2 == 0);
        return leftToRight ? posInRow : (sideLength - 1 - posInRow);
    }
        ...
        for (int i = 0; i < sideLength; i++) {
            for (int j = 0; j < sideLength; j++) {
                ...
                if (grid[i][j].hasObstacle()) {
                    ...
                    cellContent = (obstacle.getObstacleType() == ObstacleType.SNAKE)
                            ? "🐍" + obstacle.getDest()
                            : "🪜" + obstacle.getDest();
                }
